  ...
  const handleFileChange = (e) => {
    ...
    fetch('http://127.0.0.1:8000/api/detect/', {
      ...
      .then(data => {
        ...
        // const newImages = Array.from(data.detections).map(() => []);
        // console.log("==newImages",newImages)
        // const newImageRefs = Array.from(data.detections).map(() => createRef());
        // const newCanvasRefs = Array.from(data.detections).map(() => createRef());
        ...
        // Array.from(data.detections).forEach((file, index) => {
