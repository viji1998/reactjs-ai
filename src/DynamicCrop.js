...
import HighlightCoordinates from "./utils/HighlightCoordinates";

const PYTHON_API_URL = process.env.REACT_APP_PYTHON_URL;
...
const DynamicValues = () => {
  ...
  const handleFileChange = (e) => {
    ...
    fetch(`${PYTHON_API_URL}`, {
      ...
      .then(data => {
        ...
        img.onload = () => {
          // Once the image is loaded, crop it based on each detection
          const croppedImages = data.detections.map((detection) => {
            ...
            return canvas.toDataURL(); // Convert the canvas to a data URL (base64 image)
          });

          // Set the cropped images to state
          setCroppedImages(croppedImages);
          URL.revokeObjectURL(objectURL); // Clean up the object URL
        };

        if (Array.isArray(data.detections)) {
          // Create references and image data based on the number of detections
          const newImages = data.detections.map(() => null); // Initialize with null
          const newImageRefs = data.detections.map(() => createRef());
          const newCanvasRefs = data.detections.map(() => createRef());

          // Update state with new references
          setImages(newImages);
          imageRefs.current = newImageRefs;
          canvasRefs.current = newCanvasRefs;
          setCurrentImageIndex(0); // Start with the first image

          data.detections.forEach((_, index) => {
            ...
            if (files instanceof Blob) {
              const reader = new FileReader();
              reader.onload = () => {
                setImages([reader.result]); // Assume only one image is processed
              };
              reader.readAsDataURL(files); // Assuming you're reading the uploaded file
            } else {
              console.error('Provided files is not a Blob or File');
            }

          });
        } else {
          console.error('data.detections is not an array or is undefined');
        }
      })
      .catch(error => {
      ...
      const payload = croppedImages.map((imageArray, index) => {
        return imageArray.map(async (croppedImage, idx) => {
          ...
          return {
            ...
            width: croppedImage.width,
            height: croppedImage.height,
            tags: croppedImage.label, // Include tags from cropped image
  ...
  //   Array.isArray(data.detections).forEach((croppedImage) => {
  //     const ctx = canvasRefs.current[index].current.getContext("2d");
  //     const rect = imageRefs.current[index].current.getBoundingClientRect();
  //     const { naturalWidth, naturalHeight } = imageRefs.current[index].current;
   ...
   <CTableDataCell>{formData.mainImageId}</CTableDataCell> */}
          ...
          <div>
            ...
            {file && detections.length > 0 && (
              <HighlightCoordinates file={file} detections={detections} />
            )}
          </div>
        )}
        <CRow>
            ...
            <CCol key={index}>
              ...
              <CTable style={{ borderCollapse: "collapse", width: "100%" }}>
                ...
                <tbody>
                  ...
                  {/* [{"label":"Hoodies","confidence":"0.98","coordinates":{"x":77,"y":215,"width":1341,"height":1232}},{"label":"T-shirts","confidence":"0.81","coordinates":{"x":1443,"y":1295,"width":1593,"height":1028}}] */}

                  {/* {croppedImages[index]?.map((croppedImage, idx) => ( */}
                  {data.detections?.map((croppedImage, idx) => (
                    ...
                    >
                      ...
                      <CTableDataCell>
                        <img
                          ...
                          src={croppedImages[idx]} // Bind the cropped image URL here
                      ...
                      <CTableDataCell>{croppedImage.coordinates.x}</CTableDataCell>
                      <CTableDataCell>{croppedImage.coordinates.y}</CTableDataCell>
                      <CTableDataCell>{croppedImage.coordinates.width}</CTableDataCell>
                      <CTableDataCell>{croppedImage.coordinates.height}</CTableDataCell>
                      <CTableDataCell>
                        <input
                          type="text"
